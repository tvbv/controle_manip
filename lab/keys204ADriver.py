...
class Keys204A():
    ...
    def __init__(self, adress, backend = '@ivi'):
        ...
        print("Connected Device : " + self.__do_query_string("*IDN?"))
    
    def __do_command(self,command):
        self.inst.WriteString("%s" % command, True)
        self.__check_instrument_errors(command)
        
    def __do_query_string(self,query):
        """
        private method, enables querying easily the scope
        ----------
        query : str
    ...
    def set_trigger(self,trigger_channel = 1,trigger_sweep = None ,trigger_level = None, print_output = 0):
        ...
        if trigger_sweep != None:
            self.__do_command(":TRIGger:SWEep "+trigger_sweep)

        self.__do_command(":TRIGger:EDGE:SOURce CHANnel" + str(trigger_channel))
        
        if trigger_level !=None:
            self.__do_command(":TRIGger:LEVel CHANnel" + str(trigger_channel)+","+str(trigger_level))

        
        if print_output == 1:
            qresult = self.__do_query_string(":TRIGger:SWEep?")
            print("Trigger sweep: %s" % qresult)
            qresult = self.__do_query_string(":TRIGger:EDGE:SOURce?")
            print("Trigger edge source: %s" % qresult)
            qresult = self.__do_query_string(":TRIGger:LEVel? CHANnel"+ str(trigger_channel))
    ...
    def time_base(self,time_scale, time_ref,print_output = 0):
        ...
        self.__do_command(":TIMebase:SCALe " + str(time_scale))
        self.__do_command(":TIMebase:POSition 0.0")
        self.__do_command(":TIMebase:REFerence:PERCent " + str(time_ref))
        
        if print_output == 1:
            qresult = self.__do_query_string(":TIMebase:SCALe?")
            print("Timebase scale: %s" % qresult)
            qresult = self.__do_query_string(":TIMebase:POSition?")
            print("Timebase position: %s" % qresult)
            qresult = self.__do_query_string(":TIMebase:REFerence:PERCent?")
    ...
    def set_channels(self, channels=[1,2,3,4], displays = [1,1,1,1], y_scales = [1,1,1,1], offsets = [0,0,0,0], probes = [None,None,None,None] , input_couplings = ["DC",'DC','DC','DC'],print_output = 0):
        ...
        for i in range(len(channels)):
            ...
            print(i)
            if probes[i] != None : 
                self.__do_command(":CHANnel"+ str(c)+":PROBe "+str(probes[i]))
            if displays[i] != None :
                self.__do_command(":CHANnel"+ str(c)+":DISPlay "+str(displays[i]))
            if y_scales[i] != None :
                self.__do_command(":CHANnel" +str(c)+ ":SCALe " + str(y_scales[i]))
            
            if offsets[i] != None :
                self.__do_command(":CHANnel" +str(c)+ ":OFFSet "+str(offsets[i]))
            
            if input_couplings[i] != None :
                self.__do_command(":CHANnel"+ str(c)+":INPut "+ input_couplings[i])
            
            if print_output == 1:
                qresult = self.__do_query_string(":CHANnel"+ str(c)+":PROBe?")
                print("Channel "+ str(c)+" probe attenuation factor: %s" % qresult)
                qresult = self.__do_query_string(":CHANnel"+ str(c)+":DISPlay?")
                print("Channel "+ str(c)+" display: %s" % qresult)
                qresult = self.__do_query_string(":CHANnel" +str(c)+ ":SCALe?")
                print("Channel " +str(c)+ " vertical scale: "+qresult)
                qresult = self.__do_query_string(":CHANnel" +str(c)+ ":OFFSet?")
                print("Channel " +str(c)+ " offset: "+ qresult)
                qresult = self.__do_query_string(":CHANnel"+str(c)+":INPut?")
